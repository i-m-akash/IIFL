-- Accelbiz: match calls by agent_id OR by lead reference_id in dump_lead_info
-- (covers rows where fact tables have agent_id NULL but leads were imported with correct agent_id)
...
SET
  ...
  WHERE f.agent_id = $1
     OR EXISTS (
       SELECT 1 FROM accelbiz_data.dump_lead_info d
       WHERE d.reference_id = f.reference_id AND d.agent_id = $1
     )
